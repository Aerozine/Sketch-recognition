...
double plDistanceToSegment(Point p, Point p1, Point p2)
{
     ...
     *then the distance to the extremity is
     *calculated, otherwise, the distance to
     *the previously calculated point is calculated
     */
    if (param < 0)
    {
        tmp.x = p1.x;
...
{
    /*
     * Since we can't be sure that the coordinates are positive,
     * we allocate an array of boolean values to know if a point should be deleted
     */
